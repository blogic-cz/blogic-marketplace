...
```typescript
export const protectedOwnerWithBillingProcedure = protectedMemberAccessProcedure
  .use(async function requiresOwnerRole(opts) {
    if (opts.ctx.member.role !== OrganizationRoles.Owner) {
  ...
  .use(async function hasBillingAccess(opts) {
    const billingInfo = await opts.ctx.db
      .select()
      .from(billingTable)
      .where(eq(billingTable.organizationId, opts.input.organizationId))
      .limit(1);

    if (!billingInfo.length) {
      throw notFoundError("Billing information not found");
    }
...
```typescript
export const protectedFlexibleAccessProcedure = protectedProcedure
  .input(
    z.object({
  ...
  .use(async function checkFlexibleAccess(opts) {
    ...
    if (input.resourceType === "project") {
      ...
      const member = await ctx.db
        ...
        .where(
          and(
            eq(membersTable.organizationId, input.resourceId),
...
```typescript
export const rateLimitedProcedure = protectedProcedure.use(async function rateLimit(opts) {
  const { ctx } = opts;
...
```typescript
export const auditedProcedure = protectedProcedure.use(async function auditLog(opts) {
...
```typescript
export const featureFlaggedProcedure = (featureFlag: string) =>
  protectedProcedure.use(async function checkFeatureFlag(opts) {
    const { ctx } = opts;
    ...
    const feature = await ctx.db
      ...
      .where(and(eq(featureFlagsTable.name, featureFlag), eq(featureFlagsTable.enabled, true)))
      .limit(1);
...
// Usage
export const router = {
  experimentalFeature: featureFlaggedProcedure("new_dashboard").query(async ({ ctx }) => {
...
export const withPermission = (permission: Permission) =>
  protectedMemberAccessProcedure.use(async function checkPermission(opts) {
    const { ctx, input } = opts;
    ...
    const userPermissions = await ctx.db
      ...
      .where(
        and(
          eq(permissionsTable.userId, ctx.session.user.id),
          eq(permissionsTable.organizationId, input.organizationId),
    ...
    if (!userPermissions.length) {
      throw forbiddenError(`Missing required permission: ${permission}`);
    }
...
export const router = {
  deleteIntegration: withPermission("integrations:delete")
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(integrationsTable).where(eq(integrationsTable.id, input.integrationId));
    }),
} satisfies TRPCRouterRecord;
```
...
```typescript
export const resilientProcedure = protectedProcedure.use(async function errorRecovery(opts) {
  const { ctx } = opts;
  const maxRetries = 3;
...
```typescript
export const businessValidatedProcedure = protectedMemberAccessProcedure
  .input(
    z.object({
      organizationId: z.string().min(1),
  ...
  .use(async function businessValidation(opts) {
    ...
    if (existing.length > 0) {
      throw badRequestError("A project with this name already exists in your organization");
    }
    ...
    if (projectCount[0].count >= 10) {
      throw badRequestError("Organization has reached maximum project limit");
    }
...
```typescript
export const transactionalProcedure = protectedProcedure.use(
...
### Don't Use Utility Function Approach

```typescript
// ❌ BAD - Utility function approach (creates inconsistent patterns)
export const createOrgMemberCheck =
  () =>
  async ({ ctx, input, next }) => {
    // Don't do this - creates inconsistent patterns
    // Hard to compose, type inference issues
  };

// ✅ GOOD - Use reusable base procedure pattern instead
export const protectedMemberAccessProcedure = protectedProcedure
  .input(Schema.standardSchemaV1(Schema.Struct({ organizationId: Schema.String })))
  .use(async function isMemberOfOrganization(opts) {
    const memberAccess = await opts.ctx.db.select().from(membersTable).where(/* check */);
    if (!memberAccess.length) throw forbiddenError("Not a member");
    return opts.next({
      ctx: { member: memberAccess[0] },
    });
  });
```

**Why avoid utility functions:**

- Breaks the procedure chain pattern
- Harder to compose with other middleware
- Type inference doesn't work as well
- Creates inconsistent patterns across codebase
