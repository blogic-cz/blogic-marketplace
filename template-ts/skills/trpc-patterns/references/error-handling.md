  ...
  createProject: protectedMemberAccessProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing.length > 0) {
        throw badRequestError("A project with this name already exists");
      }
  ...
  updateOrganizationBilling: protectedMemberAccessProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.member.role !== OrganizationRoles.Owner) {
        throw forbiddenError("Only organization owners can update billing information");
      }
...
export const router = {
  ...
  getUserByEmail: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!user.length) {
        throw notFoundError(`User with email ${input.email} not found`);
      }
  ...
  transferProjectOwnership: protectedProjectAccessProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.project.ownerId !== ctx.session.user.id) {
        throw forbiddenError("Only the project owner can transfer ownership");
      }
      ...
      const newOwner = await ctx.db
        ...
        .where(
          and(
            ...
            eq(membersTable.organizationId, ctx.project.organizationId),
          ),
        )
        .limit(1);

      if (!newOwner.length) {
        throw notFoundError("New owner must be a member of the organization");
      }
  ...
  createApiKey: protectedProjectAccessProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (keyCount[0].count >= 10) {
        throw badRequestError("Maximum API key limit reached for this project");
      }
      ...
      const existing = await ctx.db
        .select()
        .from(apiKeysTable)
        .where(and(eq(apiKeysTable.projectId, input.projectId), eq(apiKeysTable.name, input.name)))
        .limit(1);

      if (existing.length > 0) {
        throw badRequestError("An API key with this name already exists");
      }
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        ...
        throw forbiddenError(
          "Only admins and owners can invite new members. Your current role is: " + ctx.member.role,
        );
      }
      ...
      if (existing.length > 0) {
        throw badRequestError(`${input.email} is already a member of this organization`);
      }
      ...
      if (pendingInvite.length > 0) {
        throw badRequestError(`An invitation for ${input.email} is already pending`);
      }
  ...
  createOrganizationWithProject: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        throw badRequestError("Failed to create organization and project. Please try again.");
      }
    }),
} satisfies TRPCRouterRecord;
```
...
// ✅ Good
throw badRequestError("Project name must be unique within the organization");
```
...
try {
  ...
  throw badRequestError("Operation failed. Please try again.");
}
```
